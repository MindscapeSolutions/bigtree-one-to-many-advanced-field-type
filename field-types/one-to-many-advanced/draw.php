<?
	...
	foreach ($field["value"] as $entry) {
        if (!empty($field["options"]["title_column_2"])) {
            $g = sqlfetch(sqlquery("SELECT `id`,`".$field["options"]["title_column"]."`,`".$field["options"]["title_column_2"]."` FROM `".$field["options"]["table"]."` WHERE id = '".sqlescape($entry)."'"));
            if ($g) {
                $label = $g[$field["options"]["title_column"]] . ", " . $g[$field["options"]["title_column_2"]];
                $entries[$g["id"]] = $label;
            }			
        }
        else {
	...
	$list = array();
    if (!empty($field["options"]["title_column_2"])) {
        $q = sqlquery("SELECT `id`,`".$field["options"]["title_column"]."`, `".$field["options"]["title_column_2"]."` FROM `".$field["options"]["table"]."` ORDER BY $sort");
        while ($f = sqlfetch($q)) {
            $label = $f[$field["options"]["title_column"]] . ", " . $f[$field["options"]["title_column_2"]];
            $list[$f["id"]] = $label;
        }
    }
    else {
